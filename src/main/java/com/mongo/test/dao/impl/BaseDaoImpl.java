...
import com.mongo.test.util.EmptyUtil;
 ...
 * @Author:L1ANN
 * @Description:
 * @Date:Created in 下午12:23 2018/9/25
 * @Modified By:
 */
    ...
    protected List<Order> parseOrder(String order) {
        ...
        if (EmptyUtil.isNotEmpty(order)) {
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                item = fields[i].split(" ");
                if (item.length == 1) {
                    o = new Order(Sort.Direction.ASC, item[0]);
                } else if (item.length == 2) {
                    o = new Order("desc".equalsIgnoreCase(item[1]) ? Sort.Direction.DESC : Sort.Direction.ASC, item[0]);
                } else {
